...
```powershell
pip install -r xhs-apis/scripts/requirements.txt
```

2. Install Node dependencies:

```powershell
Set-Location xhs-apis/scripts
npm install
```
...
```powershell
python xhs-apis/scripts/xhs_api_tool.py list
```

4. Call an API:

```powershell
python xhs-apis/scripts/xhs_api_tool.py call pc get_note_info --params "{\"url\":\"https://www.xiaohongshu.com/explore/...\",\"cookies_str\":\"a1=...; web_session=...\"}"
```
...
- If `jsdom` or `crypto-js` errors appear, rerun `npm install` inside `xhs-apis/scripts`.
